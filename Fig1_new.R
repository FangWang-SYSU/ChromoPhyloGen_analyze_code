...
a = res_rearrange %>%
  ...
  mutate(event_num = log(event_num))
...
for(f in fl){
  ...
  tmp_data = sctc$all_node_data
  #infer_aneu_rate$file = f
  real_cna_param[f, 'mean_PDE'] = median(tmp_predict_PDE(sctc))
}

plot(real_cna_param[,2:7])
real_cna_param$real_denovo2 = real_cna_param$aneuploidy_rate+real_cna_param$geom_p
real_cna_param$real_denovo = cut(real_cna_param$real_denovo2,6, labels = c('~0.25', '~0.35', '~0.45', '~0.55', '~0.65','~0.75'))
...
real_cna_param2 = real_cna_param
param_line_data = real_cna_param2%>%group_by(real_denovo) %>% summarise(col=median(mean_PDE)) %>% mutate(real_denovo=as.numeric(real_denovo))
