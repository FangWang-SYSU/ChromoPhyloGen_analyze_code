...
dna_file = c(
  ...
  'mhcc97l',
  'lm3')
scTrace_dir = '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/MyCellLine_all/'
...
saveRDS(DNA_list, '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/DNA_sctc_all.rds')
DNA_list = readRDS('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/DNA_sctc_all.rds')


# 保存clone信息用于拆分bam
all_bc = c()
for(i in names(DNA_list)){
  tmp1 = DNA_list[[i]]
  tmp = data.frame(bc=rownames(tmp1$orig.data$all_node_data),
                   cl=i)
  tmp = tmp[grepl('f1|f2', tmp$bc), ]

  tmp_pos =tmp1$map_obj$cell_map_pos

  tmp$clone = tmp_pos[tmp$bc, 'clone']
  all_bc = rbind(all_bc, tmp)
}
all_bc$bc = gsub('f1_', '', all_bc$bc)
all_bc$bc = gsub('f2_', '', all_bc$bc)
all_bc$bc = gsub('\\.', '-', all_bc$bc)
all_bc$clone = paste0(all_bc$cl, '_', all_bc$clone)
write.table(all_bc[,1:2], '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/DNAseq_CL_BC.txt',
            row.names = F, col.names = F, quote = F, sep = '\t')

write.table(all_bc[,c(1,3)], '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/DNAseq_CL_Clone.txt',
            row.names = F, col.names = F, quote = F, sep = '\t')
...
scTrace_dir = '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/MyCellLine_all2/'
...
# tmp_x = unlist(sctc$orig.data$all_node_data[bc, grepl('^7_', colnames(sctc$orig.data$all_node_data))])
# #region_res = find_event_region(tmp_x, window_size=round(length(tmp_x)/10))
# plot(tmp_x, type = "l")
# for (interval in all_region_limit[[bc]][['7']]) {
#   rect(interval[1], min(data), interval[2], max(data), col = "gray")
# }
...
# region_res = find_event_region(x, window_size=100)



# pheatmap::pheatmap(all_rearrange_score, cluster_rows = F,cluster_cols = F)
# pheatmap::pheatmap(all_rearrange_score_pvalue, cluster_rows = F,cluster_cols = F)
# pheatmap::pheatmap(-log(all_rearrange_score_pvalue), cluster_rows = F,cluster_cols = F)
# pheatmap::pheatmap((1-all_rearrange_score_pvalue)+0, cluster_rows = F,cluster_cols = F)
#
# pheatmap::pheatmap(t(apply(all_rearrange_score_pvalue, 1, p.adjust)), cluster_rows = F,cluster_cols = F)
#
# pheatmap::pheatmap((all_limit_prop)+0, cluster_rows = F,cluster_cols = F)

####
# limit1 = CNA_mechnism$orig.rearrange_score$all_rearrange_score
# limit_chromothripsis = rownames(limit1[limit1$type == 'chromothripsis', ])
# all_limit_prop1 = as.data.frame(all_limit_prop1)#as.data.frame(CNA_mechnism$rearrange_score$all_limit_prop)#as.data.frame(CNA_mechnism$rearrange_score$all_limit_prop[limit_chromothripsis,])
# pheatmap::pheatmap(limit1)
#
# score_vec = as.numeric(as.matrix(limit1))
...
# t_pvalue_fdr_mat = matrix(t_pvalue_fdr, nrow = nrow(limit1))
# limit1[t_pvalue_fdr_mat<0.05]=0
# pheatmap::pheatmap(limit1)
#
#
# all_limit_prop1 = all_limit_prop1[order(-all_limit_prop1$`7`), ]
# pheatmap::pheatmap(all_limit_prop1, cluster_rows = F)
#
#
# cna = sctc$orig.data$all_node_data[rownames(all_limit_prop1)[1], grepl('^7_', colnames(sctc$orig.data$all_node_data)),drop=F]
#
# pheatmap::pheatmap(cna, cluster_cols = F, cluster_rows = F, show_colnames = F, show_rownames = F)
# data.frame(cna=unlist(cna), x=1:ncol(cna)) %>%
#   ggplot(aes(x=x,y=cna))+
#   geom_point(size=0.01, color='red')+
#   theme_bw()+
#   theme(panel.grid = element_blank())

# rownames(CNA_mechnism$res) = gsub('-', '\\.', rownames(CNA_mechnism$res))
# rownames(obj$map_obj$cell_map_pos) = gsub('-', '\\.', rownames(obj$map_obj$cell_map_pos))
# rownames(CNA_mechnism$res) = gsub('\\.', '-', rownames(CNA_mechnism$res))

#pde1 = predict_PDE(sctc)
#pde2 = predict_CEE(sctc)
...
RE_p=gtree_res +
  #geom_tippoint(aes(fill=group), shape=21,size=0.4,stroke=0.01)+
  aes(color=group)+
  scale_color_gradientn(colours = c('white', '#306DAA'))+
...
RE_p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树chromothripsis着色.pdf',
...
cps_p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树cps着色.pdf',
...
BFB_p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树BFB着色.pdf',
...
limit_p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树Gradual_score着色.pdf',
...
seismic_p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树seismic_score着色.pdf',
...
pde_p=gtree_res +
  aes(color=group, size=isTip)+
  scale_color_gradientn(colours = c('white', '#8DC469'))+
  theme_tree(legend.position='right')+
  labs(title = 'CEE')+
  #coord_flip()+
  #scale_x_reverse()+
...
pde_p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树CEE着色.pdf',
...
clone_p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树Clone着色.pdf',
       clone_p, width=40, height = 60, units='mm', dpi = 600)

a = cowplot::plot_grid(clone_p, RE_p,BFB_p,pde_p, nrow=1)
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树score着色.pdf',
...
#   )
# p
# ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树chromothripsis数量着色.pdf',
...
clone_names = c("clone_5", "clone_4", "clone_1", "clone_2", "clone_3")
y <- ape::rotateConstr(sctc$clone_graph_obj$clone_graph, rev(clone_names))
...
xx= y$edge.length
y$edge.length = c(375, 310+2288, 358+1684, 408, 467+3960, 424, 276+2142, 258+2465)
plot(y)
##
...
clone_rescore = clone_rescore %>%
  group_by(clone) %>%
  summarise_all(list(median)) %>% as.data.frame()
...
p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树热图.pdf',
       p, width=120, height = 80, units='mm', dpi = 600)
...
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/clone_指标比较.pdf',
       a, width=110, height = 50, units='mm', dpi = 600)
...
a = pie_1 %>%
  ...
  mutate(clone=gsub('lone_','', clone))%>%
  mutate(clone=toupper(clone))%>%
  mutate(clone=factor(clone, levels = c('C1', 'C3','C5','C2','C4'))) %>%
  ...
  theme(strip.background = element_blank(),
        ...
        axis.text = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/clone_比较chr比例.pdf',
...
a = event_coverage %>%
  ...
  theme(strip.background = element_blank(),
        ...
        axis.ticks.length=unit(0.025, "cm"),
        axis.text = element_text(size=6))
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/clone_比较event覆盖度.pdf',
       a, width=50, height = 40, units='mm', dpi = 600)
...
height_in = 70 / 25.4
dev.off()
pdf('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_DNA_cna热图.pdf', width=width_in, height=height_in)
...
p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Huh7_进化树Fn着色.pdf',
...
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Denovo_persist相关pde着色.pdf',
...
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/PDE_genome相关.pdf',
       a, width=60, height = 50, units='mm', dpi = 600)

########### PDE 与不同指标相关性 ########################
clone_col = c('clone_1'='#3B7035','clone_2'='#DEC35D','clone_3'='#DD805A', 'clone_4'='#34B6B5', 'clone_5'='#3152A3')
meta = sctc$map_obj$cell_map_pos[, c('rearrange_score','limit_num','seismic_num', 'BFB', 'pde', 'clone')]
...
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/PDE_event相关.pdf',
...
a=plot_data %>%
  ...
  theme(panel.grid = element_blank(),
        strip.background = element_blank())
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/genome与指标相关.pdf',
       a, width=180, height = 180, units='mm', dpi = 600)

ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/genome与指标相关2.pdf',
       a, width=180, height = 180, units='mm', dpi = 600)
...
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/CellProp_shareCNA.pdf', a,
...
# a
#
# ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/CellProp_Limit_score.pdf', a,
...
# 选择8号染色体
rescore = rescore[order(-rescore[,7]),]
...
cna_data$absEnd = cumsum(as.numeric(cna_data$bin_len))
cna_data$absStart = cna_data$absEnd-cna_data$bin_len
pg = plot_genome_chr(chr_num = 'chr07',bg_color = '#C9E2F4')#, start = '59900000', end='127100000')
pg=ggplot()+
  theme_bw()+
  theme(panel.grid = element_blank(),
...
pg
tmp_cna_data = cna_data[cna_data$chrom=='7',]
tmp_seg_data = seg_data[seg_data$chr=='7',]
...
b1 = plot_genome_chr(chr_num = 'chr07',bg_color = 'white')+#, start = '59900000', end='127100000')+
  labs(y='CN')+
  lims(y=c(0,7))+
  geom_segment(aes(x=chrompos,y=integerCNV, xend=end,yend=integerCNV),
               data=cna_data[cna_data$chrom=='7',], color='red',linewidth=0.5)+
...
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Seismic展示f2_GGGCATCAGGCATGGT-1_chr7.pdf', p_seismic,
       width=80, height=60, units='mm', dpi = 600, bg = 'transparent')

ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Seismic展示f2_GGGCATCAGGCATGGT-1_7_59900000_127100000.pdf', p,
       width=80, height=60, units='mm', dpi = 600, bg = 'transparent')
...
rescore = rescore[order(-rescore[,4]),]
pheatmap::pheatmap(rescore, cluster_rows = F, cluster_cols = F)

#
bc = rownames(rescore)[2]
bc
#'#C9E2F4', '#DFEDD7'
prefix = 'huh7'
fn = 'f1'
...
tmp_seg_data = seg_data[seg_data$chr=='4',]
tmp_cna_data = cna_data[cna_data$chrom=='4',]
...
a =pg+
  geom_segment(aes(x=chrompos/1000/1000,y=ratio, xend=end/1000/1000,yend=ratio), data=cna_data[cna_data$chrom=='4',], color='black',linewidth=0.3)+
  #geom_point(aes(x=absStart,y=ratio), data=cna_data, color='black',size=0.01)+
  geom_segment(aes(x=start/1000/1000,y=ratio, xend=end/1000/1000,yend=ratio), data=tmp_seg_data, color='black',linewidth=0.5)+
  scale_x_continuous(limits=c(0, max(tmp_seg_data$end)/1000/1000),
                     expand = expansion(mult = c(0,0)))+
...
b = plot_genome_chr(chr_num = 'chr04',bg_color = 'white')+#, start = '123800000', end='170100000')+
  #labs(y='CN')+
  lims(y=c(0,7))+
  geom_segment(aes(x=chrompos,y=integerCNV, xend=end,yend=integerCNV),
               data=cna_data[cna_data$chrom=='4',], color='red',linewidth=0.5)+
...
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Limit展示f1_AGGTCATAGGCTCTTA-1_chr4.pdf', p_gradual,
       width=80, height=60, units='mm', dpi = 600, bg = 'transparent')
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Limit展示f1_TCGCGTTTCATCGCAA-1_4_123800000_170100000.pdf', p,
       width=80, height=60, units='mm', dpi = 600, bg = 'transparent')

ab = cowplot::plot_grid(p_seismic, p_gradual, nrow=1, align = 'h')
ab
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/模式展示.pdf', ab,
...
a = cell_ITH_mat %>%
  ...
  geom_violin(aes(fill=clone), size=0.001, width=1)+
  geom_boxplot(width=0.1, outlier.shape = NA, size=0.001)+
  stat_compare_means(comparisons = list(c('C1', 'C2'),
                                        c('C1', 'C3')), label = 'p.signif', size=2)+
  scale_fill_manual(values = clone_col)+
  ...
  theme(strip.background = element_blank(),
        ...
        axis.text = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/Clone_ITH.pdf', a,
...
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/PDE_ITH_相关clone.pdf',
       a, width=50, height = 46, units='mm', dpi = 600)
...
## 保存数据用于其他建tree方法
# medalt
save_data = sctc$orig.data$all_node_data[grepl('f', rownames(sctc$orig.data$all_node_data)), ]
save_data = t(save_data)
tmp_pos = t(sapply(rownames(save_data), function(x)strsplit(x,'_')[[1]][1:2]))
colnames(tmp_pos) = c('chr', 'absposstart')
save_data = cbind(tmp_pos, save_data)
save_data = save_data[!duplicated(paste0(save_data[,1], save_data[,2])),]
write.table(save_data, '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/huh7_medalt.tsv', quote = F, sep='\t', row.names = F)
#medicc2
save_data = sctc$orig.data$all_node_data[grepl('f', rownames(sctc$orig.data$all_node_data)), ]
save_data = t(save_data)
tmp_pos = t(sapply(rownames(save_data), function(x)strsplit(x,'_')[[1]][1:3]))
colnames(tmp_pos) = c('chr', 'absposstart', 'end')
save_data = cbind(tmp_pos, save_data)
save_data = reshape2::melt(as.data.frame(save_data), id=c('chr', 'absposstart', 'end'))
save_data = save_data[, c(4,1,2,3,5)]
colnames(save_data) = c('sample_id','chrom', 'start', 'end', 'cn')
save_data$chrom = paste0('chrom', save_data$chrom)
write.table(save_data, '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/huh7_medicc2.tsv',quote = F, sep='\t', row.names = F)
#sitka
save_data = sctc$orig.data$all_node_data[grepl('f', rownames(sctc$orig.data$all_node_data)), ]
save_data = t(save_data)
tmp_pos = t(sapply(rownames(save_data), function(x)strsplit(x,'_')[[1]][1:3]))
...
for(i in 1:nrow(tmp_pos)){
  curr_chr = tmp_pos[i, 1]
  curr_start = as.numeric(tmp_pos[i, 2])
  curr_end = as.numeric(tmp_pos[i, 3])
？？？？？？？？
  new_res
}
...
write.table(save_data, '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/huh7_sitka.tsv',quote = F, sep=',')

##
sim_data = sctc$orig.data$all_node_data[grepl('f', rownames(sctc$orig.data$all_node_data)), ]
sim_data = t(sim_data)
...
#####
NJ_clone = get_clone(NJ_tree, min_k = 5)
MP_clone = get_clone(MP_tree, min_k = 5)
ML_clone = get_clone(ML_tree, min_k = 5)

cell_ITH_mat2 = cell_ITH_mat
cell_ITH_mat2$clone = as.character(NJ_clone[cell_ITH_mat2$c1,'clone'])

a = cell_ITH_mat2 %>%
  mutate(clone=fct_reorder(clone, -cell_ITH, median)) %>%
  ggplot(aes(x=clone, y=cell_ITH))+
  geom_violin(aes(fill=clone), size=0.1, width=1)+
  geom_boxplot(width=0.2, outlier.shape = NA, size=0.1)+
  scale_fill_manual(values = clone_col)+
  theme_classic()+
...
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//PDE_Group_ITH.pdf',a,
...
a = plot_df[,c('pde_group', 'limit_num', 'seismic_num')] %>%
  ...
  theme(strip.background = element_blank(),
        ...
        axis.text = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/PDE_event_prop.pdf',a,
...
a=plot_df[,c('pde_group', 'limit_num', 'seismic_num')] %>%
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/PDE_event_prop2.pdf',a,
...
a = plot_df[,c('pde_group', 'limit_num', 'seismic_num')] %>%
  reshape2::melt(id='pde_group') %>%
  mutate(variable=factor(variable, levels = c('seismic_num','limit_num'))) %>%
  mutate(value2=cut(value, breaks = c(-Inf,5,10,15,20,Inf), labels=c('<5', '<10', '<15', '<20', '<24'))) %>%
  ...
  scale_fill_manual(values = c('<5'='#FFECB3FF','<10'='#FFD54FFF','<15'='#FFA000FF', '<20'='#FF6F00FF', '<24'='#2D6699'))+
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/PDE_event_prop3.pdf',a,
...
layer_data(a)
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/clone_CPS_构成.pdf',
...
layer_data(a)
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/clone_PDE_构成.pdf',
       a, width=110, height = 80, units='mm', dpi = 600)
...
a=xx %>%
  ...
  theme_void()
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/huh7_clonealign_UMAP.pdf',
...
clone_pos['clone_2', c('x', 'y')] = findPointD(clone_pos['clone_2', 'xend'],clone_pos['clone_2', 'yend'], clone_pos['clone_2', 'x'],clone_pos['clone_2', 'y'], 310,2288)
clone_pos['clone_3', c('x', 'y')] = findPointD(clone_pos['clone_3', 'xend'],clone_pos['clone_3', 'yend'], clone_pos['clone_3', 'x'],clone_pos['clone_3', 'y'], 358,1684)
...
clone_pos['clone_4', c('x', 'y')] = findPointD(clone_pos['clone_4', 'xend'],clone_pos['clone_4', 'yend'], clone_pos['clone_4', 'x'],clone_pos['clone_4', 'y'], 276,2142)
...
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_clonealign_Tree2.pdf',p,
       width=60, height=60, units='mm', dpi = 600, bg = 'transparent')
...
p = clone_num %>%
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_clone_比例柱图.pdf',p,
       width=40, height=40, units='mm', dpi = 600, bg = 'transparent')
...
# #euc_dist = cor(t(srt_pca))
# cell_ITH_dist = euc_dist[rownames(srt_pca_center),colnames(srt)]
# #cell_ITH = apply(cell_ITH, 1, mean)
# cell_ITH = c()
# for(i in rownames(srt_pca_center)){
#   x1 = cell_ITH_dist[i, rownames(srt@meta.data[srt$clonealign==i, ])]
#   x2 = cell_ITH_dist[setdiff(rownames(srt_pca_center),i), rownames(srt@meta.data[srt$clonealign==i, ])]
#   cv = x1/apply(x2, 2, sum)
#   cell_ITH = c(cell_ITH, x1)
# }
# cell_ITH = apply(cell_ITH_dist, 1, var)
...
a = cell_ITH %>%
  ...
  #geom_jitter()+
  stat_compare_means(comparisons = list(c('clone_1', 'clone_2')), label = 'p.signif')+
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_Clone_ITH.pdf',a,
       width=50, height=40, units='mm', dpi = 600, bg = 'transparent')
...
a =tmp_meta %>%
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_Clone_cytotrace.pdf',a,
       width=50, height=40, units='mm', dpi = 600, bg = 'transparent')
...
a =srt@meta.data %>%
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_Clone_CC.pdf',a,
       width=50, height=40, units='mm', dpi = 600, bg = 'transparent')
...
a=DoHeatmap(srt2, features = c(rownames(clone_marker)[1:10], tail(rownames(clone_marker), 10)),
  ...
  theme(legend.text = element_text(size=4),
        ...
        axis.text = element_text(size=6),)
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_alignHP.pdf',a,
...
a=a +
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_Clone1_GO.pdf',a,
...
a=a +
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_Clone2_GO.pdf',a,
       width=120, height=40, units='mm', dpi = 600, bg = 'transparent')
...
a = cell_ITH %>%
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_Clone_ITH_Monocle.pdf',a,
       width=50, height=40, units='mm', dpi = 600, bg = 'transparent')
...
a =pData(FC_cds) %>%
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_Clone_CC_monocle.pdf',a,
       width=50, height=40, units='mm', dpi = 600, bg = 'transparent')
...
a =pData(FC_cds) %>%
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//huh7_Clone_cytotrace_moncle.pdf',a,
       width=50, height=40, units='mm', dpi = 600, bg = 'transparent')
...
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/PDE_ITH_相关clone_RNA.pdf',
       a, width=70, height = 50, units='mm', dpi = 600)
...
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/PDE_Cytotrace_相关clone_RNA.pdf',
       a, width=60, height = 50, units='mm', dpi = 600)
...
p=p+  theme_void() +
  ...
  coord_flip()+scale_x_reverse()
p
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7//clone_tree_PDE着色.pdf',p,
...
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/PDE_genome_相关clone.pdf',
       a, width=70, height = 50, units='mm', dpi = 600)
...
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图_huh7/PDE_F2_相关clone.pdf',
       a, width=70, height = 50, units='mm', dpi = 600)
