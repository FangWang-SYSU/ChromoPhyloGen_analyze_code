...
#### 计算score ######
scTrace_dir = '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/TNBC_sub2_修改算法//'
TNBC_list_seg = list()
for(prefix in dna_file){
  print(prefix)
  ...
  TNBC_list_seg[[prefix]] = all_seg_len
}
saveRDS(TNBC_list_seg, '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/TNBC_list_seg.rds')
TNBC_list_seg = readRDS('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/TNBC_list_seg.rds')
...
for(prefix in dna_file){
  ...
  all_limit_prop = read.table(glue('{scTrace_dir}/{prefix}limit_score.txt'), header = T, sep=',', row.names = "X")
  all_rearrange_score_pvalue = read.table(glue('{scTrace_dir}/{prefix}re_score_pvalue.txt'), header = T, sep=',', row.names = "X")
  all_seg_len = TNBC_list_seg[[prefix]]
  # all_seg_len = c()
  # for(cell in rownames(sctc$orig.data$all_node_data)){
  #   print(cell)
  #   tmp_cell_len = c()
  ...
  CNA_mechnism$CPS_num = rowSums(all_rearrange_score_pvalue<pvalue_thr& all_rearrange_score>0 & all_seg_len>min_seg_num)
  CNA_mechnism$limit_num = rowSums(all_rearrange_score_pvalue<pvalue_thr & all_limit_prop>0.5& all_rearrange_score>0 & all_seg_len>min_seg_num)
  CNA_mechnism$seismic_num = rowSums(all_rearrange_score_pvalue<pvalue_thr & all_limit_prop<=0.5& all_rearrange_score>0 & all_seg_len>min_seg_num)
  CNA_mechnism$BFB = log1p(CNA_mechnism$BFB)# / ncol(sctc$orig.data$all_node_data)
  #
  all_rearrange_score2 = all_rearrange_score
  all_rearrange_score2[!(all_rearrange_score_pvalue<pvalue_thr & all_limit_prop>0.5& all_rearrange_score>0 & all_seg_len>min_seg_num)] = NA
  ...
  ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/进化树不同指标着色/tree_', prefix,'.pdf'),
  ...
  ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/进化树不同指标着色/hp_', prefix,'.pdf'),
         p2, width=160, height = 160, units='mm', dpi = 600)

  TNBC_list_with_score[[prefix]] = sctc
}

saveRDS(TNBC_list_with_score, '/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/TNBC_sctc_all_with_score_修改算法.rds')
TNBC_list_with_score = readRDS('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/data/TNBC_sctc_all_with_score_修改算法.rds')
...
#ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_umap_patient.pdf'),
#       a, width=76, height = 46, units='mm', dpi = 600)
#ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_umap_cee.pdf'),
...
a+b+c+d
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_umap.pdf'),
...
e=umap_res_df %>%
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
e
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_umap2.pdf'),
...
ae = umap_res_df %>%
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
ae
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_umap3.pdf'),
...
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_group_指标.pdf'),
...
height_in = 60 / 25.4
dev.off()
pdf('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_PDE分组异质性热图.pdf', width=width_in, height=height_in)
...
gtree = ggtree(gtree_df,size=0.1)+
  ...
  scale_x_reverse()
gtree
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_tree_hp.pdf'),
...
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone累计分布.pdf'), a,
...
for(prefix in dna_file){
  ...
  ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/指标与基因组变异相关性/',prefix,'.pdf'),
...
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/指标与基因组变异相关性/Allpatient.pdf'),
...
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/指标与基因组变异相关性/ALL.pdf'),
...
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/指标与基因组变异相关性_染色体构成.pdf'),
...
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/指标与PDE相关性/Allpatient.pdf'),
...
a = patient_index_score %>%
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.position = 'none')
a
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/patient指标箱式图.pdf'),
...
a = patient_index_score %>%
  ...
  theme(strip.background = element_blank(),
        ...
        legend.title = element_text(size=6))
a
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/patient指标箱式图CEE.pdf'),
...
for(prefix in dna_file){
  ...
  ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/细胞累计分布/',prefix,'.pdf'), a,
...
p
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/细胞累计分布/Allpatient.pdf'),
...
a=line_area %>%
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.title.x = element_blank())
a
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/细胞累计分布/AUC.pdf'),
...
a=line_area %>%
  ...
  theme(strip.background = element_blank(),
        ...
        axis.text = element_text(size=6))
a
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/细胞累计分布/AUC2.pdf'),
...
for(prefix in dna_file){
  ...
  ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/PDE与ITH相关/',prefix,'.pdf'),
...
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/PDE与ITH相关/Allpatient.pdf'),
...
a= new_score %>%
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.position = 'none')
a
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/PDE高低分组比较指标.pdf'),
...
a= new_score %>%
  ...
  theme(strip.background = element_blank(),
        ...
        axis.text = element_text(size=4))
a
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/PDE高低分组比较指标2.pdf'),
       a, width=26, height = 32, units='mm', dpi = 600)
...
a=prop_data[,c('limit_num', 'seismic_num', 'patient', 'PDE_group', 'cell_num')] %>%
  reshape2::melt(id=c('patient', 'PDE_group', 'cell_num'), variable.name='type', value.name = "chr_num") %>%
  mutate(chr_num = chr_num ) %>%
  mutate(patient=fct_reorder(patient, -chr_num, sum)) %>%
  dplyr::group_by(patient, type, PDE_group) %>%
  dplyr::summarise(chr_num=mean(chr_num, na.rm = T)) %>%
  ggboxplot(x='type', y='chr_num', fill = 'PDE_group', add = 'jitter',
            #shape=21,
            size=0.1,
  ...
  labs(y='chr_prop')+
  scale_fill_manual(values = c('PDE_high'='#E64B35FF', 'PDE_low'='#4DBBD5FF'))+
  stat_compare_means(aes(group=PDE_group), size=2, method = 'wilcox.test')+
  scale_y_continuous(expand = expansion(mult = 0.1, add = 0))+
  theme_classic()+
  theme(strip.background = element_blank(),
        legend.position = 'none',
        ...
        axis.text = element_text(size=4))
a
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/PDE高低分组比较chr.pdf'),
...
height_in = 80 / 25.4
dev.off()
pdf('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/PDE分组异质性热图.pdf', width=width_in, height=height_in)
...
pp=p +
  ...
  theme(strip.background = element_blank(),
        ...
        axis.text = element_text(size=4))
#pp
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/PDE高低分组比较基因组累计2.pdf'),
...
a=signif_peak %>%
  ...
  theme(panel.grid = element_blank())
a
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/Gistic_Peak筛选.pdf'),
...
pp
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/PDE高低分组比较基因组累计2label.pdf'),
...
a=p +
  ...
  theme()
a
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/17qRegion.pdf'),
       a, width=100, height = 80, units='mm', dpi = 600)
...
a=boxdata2 %>%
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.title.x = element_blank())
a
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/PDE高低分组ASPSCR1.pdf'),
...
#pp
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/TCGA分组ITH差异.pdf'),
       a+b+c, width=80, height = 40, units='mm', dpi = 600)
...
for(prefix in dna_file){
  ...
  all_rearrange_score_pvalue = read.table(glue('{scTrace_dir}/{prefix}re_score_pvalue.txt'), header = T, sep=',', row.names = "X")
  all_seg_len = TNBC_list_seg[[prefix]]
  min_seg_num = quantile(all_seg_len, 0.25)
...
a=ggplot()+
  ...
  theme(strip.background = element_blank(),
        ...
        axis.text = element_text(size=4))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_比较event覆盖度2.pdf',
...
a=event_coverage %>%
  ...
  theme(strip.background = element_blank(),
        ...
        axis.text = element_text(size=4))
a
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_比较event覆盖度3.pdf',
       a, width=66, height = 36, units='mm', dpi = 600)
...
aa=event_coverage %>%
  ...
  # summarise(prop=mean(coverage), all=n()) %>%
...
a4 = bb %>%
  ...
  stat_compare_means(aes(group=factor(type, levels = c('Seismic','Gradual'))),
                     label='p.format',
  ...
  theme(strip.background = element_blank(),
        ...
        axis.text = element_text(size=4))
a4
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_比较event覆盖度5.pdf',
...
a4 = bb %>%
  ...
  theme(strip.background = element_blank(),
        ...
        axis.text = element_text(size=4))
a4
ggsave('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/clone_比较event覆盖度4.pdf',
       a4, width=86, height = 36, units='mm', dpi = 600)
...
height_in = 100 / 25.4
dev.off()
pdf('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/Metabric生存.pdf', width=width_in, height=height_in)
ggsurvplot(km,
           #pval=T,
           pval = "p-value=0.0089\nAmp=372\nNeutral=1461",
...
height_in = 100 / 25.4
dev.off()
pdf('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/TCGA生存.pdf', width=width_in, height=height_in)
ggsurvplot(km,
           #pval=T,
           pval = "p-value=0.26\nAmp=341\nNeutral=506",
           #legend = "right", #将图例移动到下方
...
ggsave(paste0('/Users/lab/wangxin/单细胞进化轨迹推断/手稿文件/第一次/原图TNBC_修改算法/PDE高低分组比较基因组累计.pdf'),
       pp, width=260, height = 100, units='mm', dpi = 600)
